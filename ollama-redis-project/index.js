...
const ragApplication = await new RAGApplicationBuilder()
    .setModel(new HuggingFace({ modelName: 'yanolja/EEVE-Korean-Instruct-10.8B-v1.0' }))
    .setCache(
        new RedisCache({
            host: this.configService.get('localhost'),
            port: this.configService.get('6379'),
            // password: this.configService.get('REDIS_PASSWORD'),
        }))
    .addLoader(new PdfLoader({ filePathOrUrl: path.resolve('(교재)안전한 금융생활을 위한 보이스피싱 대처 방법.pdf') }))
    .build();
...
// const HF_EMBEDDING_API_URL = 'https://api-inference.huggingface.co/models/bge-m3'; 
...
//           HF_EMBEDDING_API_URL,  //  임베딩 API 엔드포인트
//             {
//                 input: text,
//                 model: "bge-m3:latest"  // 벡터 임베딩에서 사용하는 모델 이름
//             },
//             {
//                 headers: {
