...
const memory_conversations_js_1 = require("../conversation/memory-conversations.cjs");
class RAGApplication {
    constructor(llmBuilder) {
        ...
        if (!this.model)
            ...
            base_model_js_1.BaseModel.setConversations(llmBuilder.getConversationsEngine() || new memory_conversations_js_1.InMemoryConversation());
        }
    ...
    async init() {
        ...
        await this.vectorDb.init({ dimensions: await rag_embedding_js_1.RAGEmbedding.getEmbedding().getDimensions() });
     ...
     */
    async query(userQuery, conversationId, customContext) {
        if (!this.model) {
        ...
        this.debug(`Query resulted in ${customContext.length} chunks after filteration; chunks from ${sources.length} unique sources.`);
